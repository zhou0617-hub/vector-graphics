...
const MESH_UPDATE_INTERVAL = 3;   // 每 3 帧重算一次三角剖分
...
export function ParticleProgress({ imageUrl, progress, onRegatherComplete }: ParticleProgressProps) {
  ...
  useEffect(() => {
    ...
    let anchors: Particle[] = [];   // 只对锚点做三角剖分
    ...
    const drawTriangleMesh = () => {
      ...
      // 用鼠标附近的三角形顶点做高亮（不是光晕，只是线条更亮）
      if (mouseX > 0 && mouseY > 0) {
        ctx.strokeStyle = 'rgba(249,207,0,0.55)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        for (let i = 0; i < tri.length; i += 3) {
          const a = tri[i];
    ...
    const render = (now: number) => {
      ...
      // 三角网格：只在 scattered / regather 阶段显示
